...
def converterCoord(entrada):
	y = [i for cont,i in enumerate(entrada) if not cont%2]
	x = [i for cont,i in enumerate(entrada) if cont%2]
	return [min(y),min(x),max(y),max(x)]
	
def iou(box1, box2):
	"""Implements intersection over union (IoU) between box1 and box2
	    
	Arguments:
	box1 -- first box, list object coordinates (x1, y1, x2, y2)
	box2 -- second box, list object coordinates (x1, y1, x2, y2)
	"""
	if len(box1) == 8:
